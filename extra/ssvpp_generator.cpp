...
void genForeach()
{
    ...
    for(auto i(2u); i < foreachCount; ++i)
    {
        ...
        output << "mAction(mLast, mData, mA0) SSVPP_IMPL_FOREACH_" << i
               << "(SSVPP_INCREMENT(mLast), mAction, mData, mA1, __VA_ARGS__)";
        output << "\n";
    }
}
...
void genForeachReverse()
{
    ...
    for(auto i(2u); i < foreachCount; ++i)
    {
        ...
        output << "mAction(" << i << ", mData, mA0) SSVPP_IMPL_FOREACH_REVERSE_"
               << i << "(mAction, mData, mA1, __VA_ARGS__)";
        output << "\n";
    }
}
...
void genEval()
{
    ...
    for(auto i(1u); i < evalDepth; ++i)
    {
        ...
        for(auto j(0u); j < 3; ++j) output << "SSVPP_IMPL_EVAL_" << dp << "(";

        output << "__VA_ARGS__";

        for(auto j(0u); j < 3; ++j) output << ")";
