...
public:
    ...
    template <EType TET>
    inline auto& getAt(std::size_t mIdx) noexcept
    {
        return castUp<ETypeBase<TET>>(*getAll<TET>()[mIdx]);
    }
    template <EType TET>
    inline const auto& getAt(std::size_t mIdx) const noexcept
    {
        return castUp<ETypeBase<TET>>(*getAll<TET>()[mIdx]);
    }
};
...
private:
    ...
    template <EType TET>
    inline void setElementValue(std::size_t mIdx, const std::string& mValue)
    {
        mgr.getAt<TET>(mIdx).set(mValue);
    }

    template <EType TET>
    auto getHelpStrForET(std::string& mStr, const Impl::ManagerElements& mMgr,
...
public:
    ...
    inline Cmd& operator+=(const std::function<void()>& mFunc)
    {
        onAction += mFunc;
        return *this;
    }
    ...
    inline bool isMainCmd() const noexcept
    {
        return mainCmd;
    }

    inline const auto& getNames() const noexcept
    {
        return names;
    }
    ...
    inline const auto& getDesc() const noexcept
    {
        return desc;
    }
};
