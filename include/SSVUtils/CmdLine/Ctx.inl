...
namespace ssvu
{
	namespace CmdLine
	{
		inline Cmd& Ctx::findCmd(const std::string& mName) const
		{
			...
			auto closestMatch(std::make_pair(NumLimits<SizeT>::max(), std::string{}));
			...
			for(const auto& s : entered)
			{
				if(beginsAsFlag(s))
				{
					...
					else
					{
						...
						if(anyOf(cmd.getAll<EType::FlagValueOpt>(), [&fName](auto mFVO){ return castUp<Impl::BaseFlag>(mFVO)->hasName(fName); }))
						{
