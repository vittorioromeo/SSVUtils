...
namespace ssvu
{
    namespace CmdLine
    {
        inline Cmd& Ctx::findCmd(const std::string& mName) const
        {
            ...
            auto closestMatch(
                std::make_pair(NumLimits<SizeT>::max(), std::string{}));
        ...
        inline Cmd& Ctx::create(
            const std::initializer_list<std::string>& mNames)
        {
            return getEmplaceUPtr<Cmd>(cmds, mNames);
        }
        ...
        inline void Ctx::process(const std::vector<std::string>& mArgs)
        {
            ...
            if(entered.empty() || beginsAsFlag(entered.front()))
                useCmdMain = true;

            auto& cmd(useCmdMain ? cmdMain : findCmd(entered.front()));
            if(!useCmdMain) entered.pop_front();

            // Find all flags or flag values, put them in cFlags or cFlagValues,
            // remove them from mArgs
            ...
            for(const auto& s : entered)
            {
                if(beginsAsFlag(s))
                {
                    ...
                    if(equalPos == std::string::npos)
                    {
                        ...
                        if(cFlags.size() > cmd.getCount<EType::Flag>())
                            throw Exception::createSignatureMismatch(
                                getForCmdPhrase(cmd), "flags",
                                toStr(cmd.getCount<EType::Flag>()));
                    }
                    else
                    {
                        auto fName(s.substr(0, equalPos));
                        auto fValue(
                            s.substr(equalPos + 1, s.size() - equalPos));

                        if(anyOf(cmd.getAll<EType::FlagValueOpt>(),
                               [&fName](auto mFVO)
                               {
                                   return castUp<Impl::BaseFlag>(mFVO)->hasName(
                                       fName);
                               }))
                        {
                            cFlagValuesOpt.emplace_back(s);
                            if(cFlagValuesOpt.size() >
                                cmd.getCount<EType::FlagValueOpt>())
                                throw Exception::createSignatureMismatch(
                                    getForCmdPhrase(cmd),
                                    "optional flag values",
                                    toStr(cmd.getCount<EType::FlagValueOpt>()));
            ...
            if(cFlagValues.size() != cmd.getCount<EType::FlagValue>())
                throw Exception::createSignatureMismatch(getForCmdPhrase(cmd),
                    "required flag values",
                    toStr(cmd.getCount<EType::FlagValue>()));
            ...
            for(auto i(cmd.getCount<EType::Arg>()); i > 0; --i)
            {
                if(entered.empty())
                    throw Exception::createSignatureMismatch(
                        getForCmdPhrase(cmd), "args",
                        toStr(cmd.getCount<EType::Arg>()));
            ...
            for(auto i(cmd.getCount<EType::ArgOpt>()); i > 0; --i)
            {
                ...
                if(cArgsOpt.size() > cmd.getCount<EType::ArgOpt>())
                    throw Exception::createSignatureMismatch(
                        getForCmdPhrase(cmd), "optional args",
                        toStr(cmd.getCount<EType::ArgOpt>()));
                entered.pop_front();
            }
            ...
            for(auto i(0u); i < cmd.getCount<EType::ArgPack>(); ++i)
            {
                ...
                {
                    if(i != cmd.getCount<EType::ArgPack>() - 1)
                        throw Exception::createArgPackInfinitePositionError();
                    for(; !entered.empty(); entered.pop_front())
                        toPack.emplace_back(entered.front());
                }
                else
                {
                    if(entered.size() < argPack.getMin())
                        throw Exception::createArgPackSignatureMismatch(
                            argPack);

                    auto clampedCount(
                        getClamped(entered.size(), 0u, argPack.getMax()));
                    for(auto iS(0u); iS < clampedCount; ++iS)
                    {
                        toPack.emplace_back(entered.front());
                        entered.pop_front();
                    }
            ...
            for(auto i(0u); i < cArgs.size(); ++i)
                cmd.setElementValue<EType::Arg>(i, cArgs[i]);
            for(auto i(0u); i < cArgsOpt.size(); ++i)
                cmd.setElementValue<EType::ArgOpt>(i, cArgsOpt[i]);
            for(const auto& f : cFlags) cmd.activateFlag(f);
            for(auto i(0u); i < cFlagValuesValues.size(); ++i)
                cmd.setElementValue<EType::FlagValue>(i, cFlagValuesValues[i]);
            for(auto i(0u); i < cFlagValuesOptValues.size(); ++i)
                cmd.setElementValue<EType::FlagValueOpt>(
                    i, cFlagValuesOptValues[i]);
