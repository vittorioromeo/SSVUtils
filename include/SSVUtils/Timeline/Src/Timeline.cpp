...
namespace ssvu
{
	SSVU_INLINE bool Timeline::isCIdxNull() const noexcept { return cIdx == nullIdx; }
	...
	SSVU_INLINE Timeline::Timeline(bool mStart) noexcept { if(!mStart) stop(); }

	SSVU_INLINE void Timeline::jumpTo(Idx mIdx) noexcept	{ cIdx = mIdx; }
	SSVU_INLINE void Timeline::start()noexcept				{ finished = false; ready = true; }
	SSVU_INLINE void Timeline::clear() noexcept				{ commands.clear(); cIdx = nullIdx; finished = true; }
	SSVU_INLINE void Timeline::stop() noexcept				{ finished = true; ready = false; }
	...
	SSVU_INLINE auto Timeline::getSize() const noexcept { return commands.size(); }
