...
struct Back
{
    ...
    inline static T inOut(T mT, T mB, T mC, T mD) noexcept
    {
        ...
        T s(1.70158);
        if((mT /= mD / T(2)) < T(1))
            return mC / T(2) * (mT * mT * (((s *= T(1.525)) + T(1)) * mT - s)) +
                   mB;
        T postFix(mT -= 2);
        return mC / T(2) *
...
struct Elastic
{
    ...
    inline static T inOut(T mT, T mB, T mC, T mD) noexcept
    {
        ...
        T postFix(a * std::pow(T(2), T(-10) * (mT -= T(1))));
        return postFix * std::sin((mT * mD - s) * (T(2) * pi) / p) * T(0.5) +
               mC + mB;
    }
};
...
inline std::common_type_t<T1, T2, T3> getEased(
    const T1& mI, const T2& mIMin, const T3& mIMax) noexcept
{
    return Easing::Impl::Dispatcher<
        std::common_type_t<T1, T2, T3>>::template getMap<TEase, TKind>(mI,
        mIMin, mIMax, mIMin, mIMax);
}
...
inline std::common_type_t<T1, T2, T3, T4, T5> getMapEased(const T1& mI,
    const T2& mIMin, const T3& mIMax, const T4& mOMin, const T5& mOMax) noexcept
{
    return Easing::Impl::Dispatcher<std::common_type_t<T1, T2, T3, T4,
        T5>>::template getMap<TEase, TKind>(mI, mIMin, mIMax, mOMin, mOMax);
}
} // namespace ssvu
