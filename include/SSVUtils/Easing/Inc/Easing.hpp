...
namespace Easing
{
    ...
    namespace Impl
    {
        ...
        struct Dispatcher
        {
            ...
            inline static T getMap(const T& mI, const T& mIMin, const T& mIMax,
            const T& mOMin, const T& mOMax) noexcept
            {
    ...
    struct Back
    {
        ...
        inline static T out(T mT, T mB, T mC, T mD) noexcept
        {
            ...
            T s(1.70158);
            return mC *
                   ((mT = mT / mD - T(1)) * mT * ((s + T(1)) * mT + s) + T(1)) +
                   mB;
        }
        inline static T inOut(T mT, T mB, T mC, T mD) noexcept
        {
            ...
            T postFix(mT -= 2);
            return mC / T(2) *
                   ((postFix)*mT * (((s *= T(1.525)) + 1) * mT + s) + T(2)) +
                   mB;
        }
    };
    ...
    struct Bounce
    {
        ...
        inline static T inOut(T mT, T mB, T mC, T mD) noexcept
        {
            ...
            return out(mT * T(2) - mD, T(0), mC, mD) * T(0.5) + mC * T(0.5) +
                   mB;
        }
    };
    ...
    struct Circ
    {
        ...
        inline static T inOut(T mT, T mB, T mC, T mD) noexcept
        {
            ...
            return mC / T(2) * (std::sqrt(T(1) - mT * (mT -= T(2))) + T(1)) +
                   mB;
        }
    };
    ...
    struct Elastic
    {
        ...
        inline static T inOut(T mT, T mB, T mC, T mD) noexcept
        {
            ...
            if(mT < 1) {
                ...
                return -T(0.5) *
                       (postFix * std::sin((mT * mD - s) * (T(2) * pi) / p)) +
                       mB;
            }
...
{
    return Easing::Impl::Dispatcher<Common<T1, T2, T3>>::template getMap<TEase,
    TKind>(mI, mIMin, mIMax, mIMin, mIMax);
}
...
const T3& mIMax, const T4& mOMin, const T5& mOMax) noexcept
{
    ...
    Common<T1, T2, T3, T4, T5>>::template getMap<TEase, TKind>(mI, mIMin, mIMax,
    mOMin, mOMax);
}
}
