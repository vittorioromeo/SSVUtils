// Copyright (mC) 2013-2015 Vittorio Romeo
...
namespace ssvu
{
    namespace Easing
    {
        ...
        namespace Impl
        {
            ...
            struct Dispatcher
            {
                template <template <typename> class TEase,
                    template <typename> class TKind>
                inline static T getMap(const T& mI, const T& mIMin,
                    const T& mIMax, const T& mOMin, const T& mOMax) noexcept
                {
                    return TKind<TEase<T>>::get()(
                        mIMin + mI, mOMin, mOMax - mOMin, mIMax - mIMin);
                }
            };
        }
        ...
        struct Back
        {
            ...
            inline static T inOut(T mT, T mB, T mC, T mD) noexcept
            {
                ...
                if((mT /= mD / T(2)) < T(1))
                    return mC / T(2) *
                               (mT * mT * (((s *= T(1.525)) + T(1)) * mT - s)) +
                           mB;
                T postFix(mT -= 2);
                return mC / T(2) *
                           ((postFix)*mT * (((s *= T(1.525)) + 1) * mT + s) +
                               T(2)) +
                       mB;
            }
        };
        ...
        struct Bounce
        {
            inline static T in(T mT, T mB, T mC, T mD) noexcept
            {
                return mC - out(mD - mT, T(0), mC, mD) + mB;
            }
            inline static T out(T mT, T mB, T mC, T mD) noexcept
            {
                SSVU_ASSERT(mD != 0);
                if((mT /= mD) < (T(1) / T(2.75)))
                    return mC * (T(7.5625) * mT * mT) + mB;
                if(mT < (T(2) / T(2.75)))
                {
            ...
            {
                if(mT < mD / T(2))
                    return in(mT * T(2), T(0), mC, mD) * T(0.5) + mB;
                return out(mT * T(2) - mD, T(0), mC, mD) * T(0.5) +
                       mC * T(0.5) + mB;
            }
        };
        ...
        struct Circ
        {
            ...
            inline static T inOut(T mT, T mB, T mC, T mD) noexcept
            {
                SSVU_ASSERT(mD != 0);
                if((mT /= mD / T(2)) < T(1))
                    return -mC / T(2) * (std::sqrt(T(1) - mT * mT) - T(1)) + mB;
                return mC / T(2) *
                           (std::sqrt(T(1) - mT * (mT -= T(2))) + T(1)) +
                       mB;
            }
        };
        ...
        struct Elastic
        {
            inline static T in(T mT, T mB, T mC, T mD) noexcept
            {
                ...
                T p(mD * T(0.3)), a(mC), s(p / T(4)),
                    postFix(a * std::pow(T(2), T(10) * (mT -= T(1))));
                SSVU_ASSERT(p != 0);

                return -(postFix * std::sin((mT * mD - s) * (T(2) * pi) / p)) +
                       mB;
            }
            inline static T out(T mT, T mB, T mC, T mD) noexcept
            {
                ...
                return (a * std::pow(T(2), T(-10) * mT) *
                            std::sin((mT * mD - s) * (T(2) * pi) / p) +
                        mC + mB);
            }
            inline static T inOut(T mT, T mB, T mC, T mD) noexcept
            {
                ...
                if(mT < 1)
                {
                    ...
                    return -T(0.5) * (postFix * std::sin((mT * mD - s) *
                                                         (T(2) * pi) / p)) +
                           mB;
                }
        ...
        struct Expo
        {
            inline static T in(T mT, T mB, T mC, T mD) noexcept
            {
                ...
                return (mT == T(0))
                           ? mB
                           : mC * std::pow(T(2), T(10) * (mT / mD - T(1))) + mB;
            }
            inline static T out(T mT, T mB, T mC, T mD) noexcept
            {
                SSVU_ASSERT(mD != 0);
            ...
            inline static T inOut(T mT, T mB, T mC, T mD) noexcept
            {
                ...
                if((mT /= mD / T(2)) < T(1))
                    return mC / T(2) * std::pow(T(2), T(10) * (mT - T(1))) + mB;
        ...
        struct Quint
        {
            ...
            {
                SSVU_ASSERT(mD != 0);
                return mC * ((mT = mT / mD - T(1)) * mT * mT * mT * mT + T(1)) +
                       mB;
            }
            inline static T inOut(T mT, T mB, T mC, T mD) noexcept
            {
                SSVU_ASSERT(mD != 0);
                if((mT /= mD / T(2)) < T(1))
                    return mC / T(2) * mT * mT * mT * mT * mT + mB;
                return mC / T(2) * ((mT -= T(2)) * mT * mT * mT * mT + T(2)) +
                       mB;
            }
        };
    }
    ...
    inline Common<T1, T2, T3> getEased(
        const T1& mI, const T2& mIMin, const T3& mIMax) noexcept
    {
        return Easing::Impl::Dispatcher<Common<T1, T2, T3>>::template getMap<
            TEase, TKind>(mI, mIMin, mIMax, mIMin, mIMax);
    }
    ...
    inline Common<T1, T2, T3, T4, T5> getMapEased(const T1& mI, const T2& mIMin,
        const T3& mIMax, const T4& mOMin, const T5& mOMax) noexcept
    {
        return Easing::Impl::Dispatcher<
            Common<T1, T2, T3, T4, T5>>::template getMap<TEase, TKind>(mI,
            mIMin, mIMax, mOMin, mOMax);
    }
}
