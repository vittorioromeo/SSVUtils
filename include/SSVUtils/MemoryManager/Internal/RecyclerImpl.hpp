...
namespace ssvu
{
    namespace Impl
    {
        ...
        using MonoRecyclerBase =
            BaseRecycler<TBase, TLHelper, MonoStorage<TBase, TLHelper>,
                         MonoRecyclerImpl<TBase, TLHelper>>;
        template <typename TBase, template <typename> class TLHelper,
                  typename TStorage>
        using PolyRecyclerBase =
            BaseRecycler<TBase, TLHelper, TStorage,
        ...
        template <typename TBase, template <typename> class TLHelper,
                  typename TStorage, typename TDerived>
        class BaseRecycler
        {
        public:
        ...
        public:
            ...
            /// emplaced.
            template <typename T = TBase, typename TC, typename... TArgs>
            inline T& getCreateEmplaceAt(TC& mContainer, SizeT mIdx,
                                         TArgs&&... mArgs)
            {
                auto itr(mContainer.emplace(std::begin(mContainer) + mIdx,
                                            create<T>(FWD(mArgs)...)));
                return castUp<T>(**itr);
            }
        };
        ...
        struct MonoRecyclerImpl final : public MonoRecyclerBase<TBase, TLHelper>
        {
            ...
            inline auto createImpl(TArgs&&... mArgs)
            {
                SSVU_ASSERT_STATIC(isSame<TBase, T>(),
                                   ...
                                   "of the same type");
                return PtrType{
                    this->storage.chunk.template create<T>(FWD(mArgs)...),
            ...
            : public PolyRecyclerBase<TBase, TLHelper, TStorage>
        {
            ...
            inline auto createImpl(TArgs&&... mArgs)
            {
                ...
                return PtrType{chunk.template create<T>(FWD(mArgs)...),
                               ChunkDeleterType{chunk}};
            }
        };
    }
}
