...
public:
    ...
    inline MMItrBase(TItrValue mValue, TArgs&&... mArgs) noexcept
        : BaseAdaptorItrRnd<TItrValue, TImpl>{mValue}, impl{FWD(mArgs)...}
    {
    }
...
using MMItrIdx = MMItrBase<TBase, std::size_t, MMItrImplIdx<TBM>>;
...
public:
    ...
    template <typename T = TBase, typename... TArgs>
    inline T& createAt(std::size_t mIdx, TArgs&&... mArgs)
    {
        return recycler.template getCreateEmplaceAt<T>(
            items, mIdx, FWD(mArgs)...);
    }

    inline decltype(auto) operator[](std::size_t mI) noexcept
    {
        return items[mI];
    }
...
private:
    RecyclerType recycler;
    Container items;
    std::size_t msize{0u}, sizeNext{0u}, capacity{0u};

    inline bool isAliveAt(std::size_t mI) const noexcept
    {
...
public:
    ...
    inline auto& getDataAt(std::size_t mI) noexcept
    {
        return items[mI];
    }
    inline const auto& getDataAt(std::size_t mI) const noexcept
    {
        return items[mI];
    }
    ...
    {
        Impl::refreshImpl(
            msize, sizeNext, [this](std::size_t mI) { return isAliveAt(mI); },
            [this](std::size_t mD, std::size_t mA) {
                using std::swap;
                swap(items[mD], items[mA]);
            },
            [this](std::size_t mD) { items.deinitAt(mD); });
    }
