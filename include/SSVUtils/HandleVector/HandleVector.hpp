...
namespace ssvu
{
    ...
    private:
        ...
        /// @brief Sets the status of the atom pointed by the mark at mMarkIdx
        /// to dead.
        ...
        inline auto& getMarkFromAtom(const Atom& mAtom) noexcept
        {
            return marks[mAtom.getMarkIdx()];
        }

        /// @brief Returns a reference to mMark's controlled atom.
        inline auto& getAtomFromMark(const Mark& mMark) noexcept
        {
    ...
    public:
        ...
        /// @details The created atom will not be used until the HandleVector is
        /// refreshed.
        inline auto createHandleFromAtom(Atom& mAtom) noexcept
        {
            return Handle<T>{*this, mAtom.getMarkIdx(),
                             getMarkFromAtom(mAtom).ctr};
        }
        ...
        inline auto& createAtom(TArgs&&... mArgs)
        {
        ...
        /// @details Dead atoms are deallocated and destroyed. Newly created
        /// atoms are now taken into account.
        ...
        /// @brief Iterates over alive data. Newly created atoms aren't taken
        /// into account.
        ...
        /// @brief Iterates over alive atoms. Newly created atoms aren't taken
        /// into account.
        template <typename TFunc>
        ...
        inline const auto& getAtomAt(HIdx mIdx) const noexcept
        {
            return atoms[mIdx];
        }

        /// @brief Returns a reference to the data at mIdx. Assumes the data is
        /// initialized.
        inline T& getDataAt(HIdx mIdx) noexcept
        {
            return getAtomAt(mIdx).getData();
        }

        /// @brief Returns a const reference to the data at mIdx. Assumes the
        /// data is initialized.
        inline const T& getDataAt(HIdx mIdx) const noexcept
        {
            return getAtomAt(mIdx).getData();
        }

        /// @brief Returns the current size of the HandleVector. Newly created
        /// atoms aren't taken into account.
        inline auto getSize() const noexcept { return size; }

        /// @brief Returns the next size of the HandleVector. Newly created
        ...
        /// @brief Returns a reference to `mData`'s atom. Assumes `mData` is a
        /// member of an atom.
        /// @details Will not work correctly if the HandleVector gets resized
        /// (either by reserving or adding elements).
        ...
        inline constexpr Atom& getAtomFromHandle(Handle<T>& mHandle) noexcept
        {
        ...
        /// @details This iterator will be invalidated if the internal storage
        /// grows.
        inline auto begin() noexcept { return ItrFast{&atoms[0]}; }

        /// @brief Returns a fast iterator pointing one after the last data.
        /// Newly created atoms aren't taken into account.
        /// @details This iterator will be invalidated if the internal storage
        /// grows.
        inline auto end() noexcept { return ItrFast{&atoms[size]}; }

        /// @brief Returns a fast iterator pointing one after the last
        /// newly-created data. Newly created atoms are taken into account.
        /// @details This iterator will be invalidated if the internal storage
        /// grows.
        inline auto endNext() noexcept { return ItrFast{&atoms[sizeNext]}; }

        /// @brief Returns a fast iterator pointing to the first data. (const
        /// version)
        /// @details This iterator will be invalidated if the internal storage
        /// grows.
        ...
        /// @brief Returns a fast iterator pointing one after the last data.
        /// Newly created atoms aren't taken into account. (const version)
        /// @details This iterator will be invalidated if the internal storage
        /// grows.
        ...
        /// @details This iterator won't be invalidated if the internal storage
        /// grows.
        inline auto beginIdx() noexcept { return ItrIdx{0, *this}; }

        /// @brief Returns an index iterator pointing one after the last data.
        /// Newly created atoms aren't taken into account.
        /// @details This iterator won't be invalidated if the internal storage
        /// grows.
        inline auto endIdx() noexcept { return ItrIdx{size, *this}; }

        /// @brief Returns an index iterator pointing one after the last
        /// newly-created data. Newly created atoms are taken into account.
        /// @details This iterator won't be invalidated if the internal storage
        /// grows.
        inline auto endIdxNext() noexcept { return ItrIdx{sizeNext, *this}; }

        /// @brief Returns an index iterator pointing to the first data. (const
        /// vesrion)
        /// @details This iterator won't be invalidated if the internal storage
        /// grows.
        ...
        /// @brief Returns an index iterator pointing to one after the last
        /// newly-created atom.
        /// @details This iterator won't be invalidated if the internal storage
        /// grows.
        inline auto endAtomNext() noexcept { return ItrAtom{sizeNext, *this}; }

        /// @brief Returns an index iterator pointing to the first atom. (const
        /// version)
        /// @details This iterator won't be invalidated if the internal storage
        /// grows.
        ...
        /// @brief Returns an index iterator pointing to one after the last
        /// newly-created atom. (const version)
        /// @details This iterator won't be invalidated if the internal storage
        /// grows.
        inline auto endAtomNext() const noexcept
        {
            return ItrAtom{sizeNext, *this};
        }
        ...
        /// @brief Returns a range using fast iterators. Newly created atoms
        /// aren't taken into account.
        /// @details This range will be invalidated if the internal storage
        /// grows.
        inline auto forFast() noexcept { return makeRange(begin(), end()); }

        /// @brief Returns a range using fast iterators. Newly created atoms
        /// aren't taken into account. (const version)
        /// @details This range will be invalidated if the internal storage
        /// grows.
        inline auto forFast() const noexcept
        {
            return makeRange(begin(), end());
        }

        /// @brief Returns a range using fast iterators. Newly created atoms are
        /// taken into account.
        /// @details This range will be invalidated if the internal storage
        /// grows.
        inline auto forNextFast() noexcept
        {
            return makeRange(begin(), endNext());
        }

        /// @brief Returns a range using fast iterators. Newly created atoms are
        /// taken into account. (const version)
        /// @details This range will be invalidated if the internal storage
        /// grows.
        inline auto forNextFast() const noexcept
        {
            return makeRange(begin(), endNext());
        }
        ...
        /// @brief Returns a range using idx iterators. Newly created atoms
        /// aren't taken into account.
        /// @details This range will be invalidated if the internal storage
        /// grows.
        ...
        /// @brief Returns a range using idx iterators. Newly created atoms are
        /// taken into account.
        /// @details This range will be invalidated if the internal storage
        /// gro-ws.
        ...
        /// @brief Returns a range for atom iteration. Newly created atoms
        /// aren't taken into account.
        /// @details This range will be invalidated if the internal storage
        /// grows.
        inline auto forAtom() noexcept
        {
            return makeRange(beginAtom(), endAtom());
        }
        ...
        /// @details This range will be invalidated if the internal storage
        /// grows.
        inline auto forAtom() const noexcept
        {
            return makeRange(beginAtom(), endAtom());
        }

        /// @brief Returns a range for atom iteration. Newly created atoms are
        /// taken into account.
        /// @details This range will be invalidated if the internal storage
        /// grows.
        inline auto forNextAtom() noexcept
        {
            return makeRange(beginAtom(), endAtomNext());
        }
        ...
        /// @details This range will be invalidated if the internal storage
        /// grows.
        inline auto forNextAtom() const noexcept
        {
            return makeRange(beginAtom(), endAtomNext());
        }
    };
