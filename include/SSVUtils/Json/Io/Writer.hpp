    ...
    namespace Json
    {
        namespace Impl
        {
            ...
            private:
                ...
                {
                    return mVal.getType() == Val::Type::TObj ||
                           mVal.getType() == Val::Type::TArr;
                }
                ...
                inline void repeatWithSeparator(TItr mBegin, TItr mEnd, TF1 mF1,
                                                TF2 mF2)
                {
                ...
                inline void write(const Obj& mObj)
                {
                    ...
                    repeatWithSeparator(std::begin(mObj), std::end(mObj),
                                        [this](auto mItr)
                                        {
                                            writeKey(mItr->first);
                ...
                inline void write(const Arr& mArr)
                {
                    ...
                    repeatWithSeparator(std::begin(mArr), std::end(mArr),
                                        [this](auto mItr)
                                        {
                                            write(*mItr);
                                        },
                ...
                inline void write(const Num& mNum)
                {
                    ...
                    {
                        case Num::Repr::IntS:
                            wOut(toStr(mNum.as<IntS>()));
                            break;
                        case Num::Repr::IntU:
                            wOut(toStr(mNum.as<IntU>()));
                            break;
                        case Num::Repr::Real:
                            wOut(toStr(mNum.as<Real>()));
                            break;
                    }
                }
            ...
            public:
                inline void write(const Val& mVal, std::ostream& mStream)
                {
                    write(mVal);
                    mStream << out;
                }
