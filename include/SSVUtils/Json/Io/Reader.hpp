    ...
    namespace Impl
    {
        ...
        private:
            ...
            inline auto getErrorSrc()
            {
                ...
                auto strMarked(std::string{std::begin(src) + iStart,
                               std::begin(src) + iDStart} +
                               " o>>> " + std::string{std::begin(src) + iDStart,
                                          std::begin(src) + iDEnd} +
                               " <<<o " + std::string{std::begin(src) + iDEnd,
                                          std::begin(src) + iEnd});

                auto strUnmarked(
                std::string{std::begin(src) + iStart, std::begin(src) + iEnd});
            ...
            inline static constexpr auto isNumStart(char mC) noexcept
            {
            ...
            inline char& getC(SizeT mIdx) noexcept
            {
                SSVU_ASSERT(mIdx >= 0 && mIdx < src.size());
                return src[mIdx];
            }
            inline char getC(SizeT mIdx) const noexcept
            {
                SSVU_ASSERT(mIdx >= 0 && mIdx < src.size());
                return src[mIdx];
            }
            ...
            inline Val parseBlnFalse()
            {
                match("false");
                return Val{false};
            }
            inline Val parseBlnTrue()
            {
                match("true");
                return Val{true};
            }
            ...
            inline Val parseNum()
            {
                ...
                Real realN(
                toNum<Real>(std::strtod(src.data() + idx, &endChar)));
            ...
            inline Val parseArr()
            {
                ...
                while(true) {
                    ...
                    std::string{"Expected either `,` or `]`, got `"} + getC() +
                    "`");
                }
        ...
        public:
            ...
            inline Val parseVal()
            {
                ...
                std::string{"No match for values beginning with `"} + getC() +
                "`");
