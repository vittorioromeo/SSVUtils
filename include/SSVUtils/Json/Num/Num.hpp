...
/// @brief Internal class dedicated to the storage of numeric
/// values.
class Num
{
    template <typename>
    friend struct Impl::NumHelper;
...
private:
    ...
    // Union setters
    inline void setIntS(const IntU& mX) noexcept
    {
        repr = Repr::IntS;
        h.init<IntS>(mX);
    }
    inline void setIntU(const IntS& mX) noexcept
    {
        repr = Repr::IntU;
        h.init<IntU>(mX);
    }
    inline void setReal(const Real& mX) noexcept
    {
        repr = Repr::Real;
        h.init<Real>(mX);
    }

    /// @brief Returns the stored value as an `IntS`. The value is
    /// copied and casted if necessary.
    ...
    /// @brief Returns the stored value as an `IntU`. The value is
    /// copied and casted if necessary.
    ...
    /// @brief Returns the stored value as a `Real`. The value is
    /// copied and casted if necessary.
...
public:
    ...
    template <typename T>
    inline Num(const T& mX) noexcept
    {
        set<T>(mX);
    }

    /// @brief Sets the stored number to `mX`. Representation may
    /// change depending on `mX`'s type.
    template <typename T>
    inline void set(const T& mX) noexcept
    {
        Impl::NumHelper<T>::set(*this, mX);
    }

    /// @brief Returns a copy of the stored number, as a `T`.
    /// Representation will not change.
    /// @details The internal number will not change. The copy will
    /// be casted, if necessary.
    template <typename T>
    inline auto as() const noexcept
    {
        return Impl::NumHelper<T>::as(*this);
    }

    /// @brief Returns the current representation type.
    inline auto getRepr() const noexcept
    {
        return repr;
    }
    ...
    inline auto operator!=(const Num& mN) const noexcept
    {
        return !(operator==(mN));
    }
};
