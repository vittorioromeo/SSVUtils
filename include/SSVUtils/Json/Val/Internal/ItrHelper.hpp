...
struct ItrHelper
{
    ...
    struct ImplAsObj
    {
        ...
        {
            return makeKVPair(mItr->first, mItr->second.template as<T>());
        }
    };
    ...
    template <template <typename> class TImpl, typename T, typename TItr>
    ...
    template <template <typename> class TImpl, typename T, typename TItr>
    inline static constexpr auto makeItrAsRange(TItr mBegin, TItr mEnd) noexcept
    {
        return makeRange(
