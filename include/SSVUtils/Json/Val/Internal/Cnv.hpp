...
#define SSVJ_DEFINE_CNV_BIG_MUTABLE(mType)                                   \
    ...
    {                                                                        \
        ...
        inline static void                                                   \
        toVal(Val& mV,                                                       \
              T&& mX) noexcept(noexcept(VRM_PP_CAT(mV.set, mType)(FWD(mX)))) \
        {                                                                    \
...
#undef SSVJ_DEFINE_CNV_SMALL_IMMUTABLE
            ...
            struct Cnv<Val, void> final
            {
                template <typename T>
                inline static void
                toVal(Val& mV, T&& mX) noexcept(noexcept(mV.init(FWD(mX))))
                {
                    mV.init(FWD(mX));
                }
            };
            ...
            // Convert enums
            template <typename T>
            struct Cnv<T, EnableIf<isEnum<RmAll<T>>()>> final
            {
                ...
                {
                    mX = T(mV.template as<Underlying<T>>());
                }
            };
            ...
            struct Cnv<char[TS]> final
            {
                ...
                inline static void fromVal(const Val& mV,
                                           char(&mX)[TS]) noexcept
                {
            ...
            struct Cnv<std::pair<T1, T2>> final
            {
                ...
                {
                    mX.first =
                        moveIfRValue<decltype(mV)>(mV[0].template as<T1>());
                    mX.second =
                        moveIfRValue<decltype(mV)>(mV[1].template as<T2>());
                }
            };
                    ...
                    tplForData(
                        [&mV](auto mD, auto& mE)
                        {
                            SSVU_ASSERT(mV.template is<Arr>() &&
                                        mV.getArr().size() > mD.getIdx());
                            mE = moveIfRValue<decltype(mV)>(
                                FWD(mV)[mD.getIdx()]
                                    .template as<RmRef<decltype(mE)>>());
                        },
                        mX);
                }
            };
            ...
            struct Cnv<std::vector<TItem>> final
            {
                ...
                inline static void fromVal(T&& mV, Type& mX)
                {
                    ...
                    for(auto i(0u); i < arr.size(); ++i)
                        mX.emplace_back(moveIfRValue<decltype(mV)>(
            ...
            struct Cnv<TMap<TKey, TValue, TExtra...>> final
            {
                ...
                inline static void toVal(Val& mVal, T&& mX)
                {
                    mVal = Arr{};
                    for(auto& p : mX)
                        mVal.getArr().emplace_back(
                            moveIfRValue<decltype(mX)>(p));
                }
                template <typename T>
                inline static void fromVal(T&& mVal, Type& mX)
                {
                    for(auto p :
                        mVal.template forArrAs<std::pair<TKey, TValue>>())
                    {
                        mX[p.first] = p.second;
                    }
                }
            ...
            struct Cnv<TItem[TS]>
            {
                ...
                inline static void fromVal(T&& mV, Type& mX)
                {
                    ...
                    for(auto i(0u); i < TS; ++i)
                        mX[i] = moveIfRValue<decltype(mV)>(
                            mV[i].template as<TItem>());
                }
            };
