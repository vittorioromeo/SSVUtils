...
#define SSVJ_IMPL_SRLZ_ARR_IMPL_SEP_ARG_STEP(mIdx, mData, mArg)	mData . mArg VRM_PP_COMMA_IF(mIdx)
#define SSVJ_IMPL_SRLZ_ARR_IMPL_SEP_ARG(mX, ...)				VRM_PP_FOREACH_REVERSE(SSVJ_IMPL_SRLZ_ARR_IMPL_SEP_ARG_STEP, mX, __VA_ARGS__)

// Serialize members to obj with same name
#define SSVJ_IMPL_SRLZ_OBJ_AUTO_IMPL_SEP_ARG_STEP(mIdx, mData, mArg)	VRM_PP_TOSTR(mArg), mData . mArg VRM_PP_COMMA_IF(mIdx)
#define SSVJ_IMPL_SRLZ_OBJ_AUTO_IMPL_SEP_ARG(mX, ...)					VRM_PP_FOREACH_REVERSE(SSVJ_IMPL_SRLZ_OBJ_AUTO_IMPL_SEP_ARG_STEP, mX, __VA_ARGS__)

// Wrapper macro to avoid repetition
#define SSVJ_IMPL_CNV_WRAPPER(mType, mTemplateArgs, mBody) \
	SSVJ_CNV_NAMESPACE() \
	{ \
		template< VRM_PP_TPL_EXPLODE(mTemplateArgs) > SSVJ_CNV(mType, mV, mX) \
		{ \
...
/// @macro Serialize/deserialize the specified member `mArg` of `mX` to a JSON value in `mV`.
#define SSVJ_SRLZ_VAL(mV, mX, mArg) ::ssvj::cnv(mV, mX . mArg)

/// @macro Serialize/deserialize all specified members of `mX` to a JSON array in `mV`.
#define SSVJ_SRLZ_ARR(mV, mX, ...) ::ssvj::cnvArr(mV, SSVJ_IMPL_SRLZ_ARR_IMPL_SEP_ARG(mX, __VA_ARGS__))

/// @macro Serialize/deserialize all specified members of `mX` to a JSON object in `mV`.
/// @details The object keys are the member names.
#define SSVJ_SRLZ_OBJ_AUTO(mV, mX, ...) ::ssvj::cnvObj(mV, SSVJ_IMPL_SRLZ_OBJ_AUTO_IMPL_SEP_ARG(mX, __VA_ARGS__))

/// @macro Define a JSON converter for type `mType`, that serializes/deserializes the member `mArg` to a JSON value.
#define SSVJ_CNV_VAL(mType, mArg) \
	SSVJ_IMPL_CNV_WRAPPER(mType, (), (SSVJ_SRLZ_VAL(mV, mX, mArg);))

/// @macro Define a JSON converter for type `mType`, that serializes/deserializes the specified members to a JSON array.
#define SSVJ_CNV_ARR(mType, ...) \
	SSVJ_IMPL_CNV_WRAPPER(mType, (), (SSVJ_SRLZ_ARR(mV, mX, __VA_ARGS__);))

/// @macro Define a templatized JSON converter for type `mType`, that serializes/deserializes the specified members to a JSON array.
/// @details Used for template types. Specify template arguments in `mTemplateArgs`.
#define SSVJ_CNV_ARR_TEMPLATE(mTemplateArgs, mType, ...) \
	SSVJ_IMPL_CNV_WRAPPER(mType, (mTemplateArgs), (SSVJ_SRLZ_ARR(mV, mX, __VA_ARGS__);))

/// @macro Define a JSON converter for type `mType`, that serializes/deserializes the specified members to a JSON objects.
/// @details The object keys are the member names.
