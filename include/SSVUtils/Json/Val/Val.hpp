...
#undef SSVJ_DEFINE_VAL_GETTER
            ...
            public:
                ...
                inline void set(T&& mX) noexcept(noexcept(
                    Impl::Cnv<RmAll<T>, void>::toVal(std::declval<Val&>(),
                                                     FWD(mX))))
                {
                    deinitCurrent();
                ...
                /// `Obj`.
                template <typename T>
                inline decltype(auto) getIfHas(const Key& mKey,
                                               const T& mDef) const
                {
                ...
                {
                    return VIH::makeItrObjRange<T>(std::begin(getObj()),
                                                   std::end(getObj()));
                }
                template <typename T>
                inline auto forUncheckedObjAs() const noexcept
                {
                    return VIH::makeItrObjRange<T>(std::cbegin(getObj()),
                                                   std::cend(getObj()));
                }
                template <typename T>
                inline auto forUncheckedArrAs() noexcept
                {
                    return VIH::makeItrArrRange<T>(std::begin(getArr()),
                                                   std::end(getArr()));
                }
                template <typename T>
                inline auto forUncheckedArrAs() const noexcept
                {
                    return VIH::makeItrArrRange<T>(std::cbegin(getArr()),
                                                   std::cend(getArr()));
                }
