...
namespace ssvu
{
    ...
    {
        return std::any_of(std::begin(mContainer), std::end(mContainer),
                           mPredicate);
    }
    ...
    {
        return std::rotate(std::begin(mContainer), mNewBegin,
                           std::end(mContainer));
    }
    ...
    {
        return std::find_if(std::begin(mContainer), std::end(mContainer),
                            mPredicate);
    }
    ...
    {
        return std::remove(std::begin(mContainer), std::end(mContainer),
                           mValue);
    }
    ...
    {
        return std::remove_if(std::begin(mContainer), std::end(mContainer),
                              mPredicate);
    }
    ...
    {
        return std::lower_bound(std::begin(mContainer), std::end(mContainer),
                                mValue);
    }
    ...
    {
        mContainer.erase(removeIf(mContainer, mPredicate),
                         std::end(mContainer));
    }
    ...
    inline void trimL(TC& mContainer, TP mPredicate)
    {
        mContainer.erase(std::begin(mContainer),
                         std::find_if_not(std::begin(mContainer),
    ...
    namespace Impl
    {
        ...
        template <typename T, typename TC, typename TK, typename TM,
                  typename... TArgs>
        inline T& getEmplaceUPtrMapImpl(TC& mContainer, TK&& mKey,
                                        TArgs&&... mArgs)
        {
            return castUp<T>(
                *(*(mContainer.emplace(FWD(mKey), TM::template make<TArgs...>(
                                                      FWD(mArgs)...))
                        .first))
                     .second);
        }
    ...
    {
        std::shuffle(std::begin(mContainer), std::end(mContainer),
                     ssvu::getRndEngine());
    }
