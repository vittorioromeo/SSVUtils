...
namespace Impl
{
/// @brief Internal implementation method for std::unique_ptr emplacement in
/// linear containers.
...
/// @brief Internal implementation method for std::unique_ptr emplacement in map
/// containers.
template <typename T, typename TC, typename TK, typename TM, typename... TArgs>
inline T& getEmplaceUPtrMapImpl(TC& mContainer, TK&& mKey, TArgs&&... mArgs)
{
    return castUp<T>(*(
        *(mContainer
                .emplace(FWD(mKey), TM::template make<TArgs...>(FWD(mArgs)...))
                .first))
                          .second);
}
} // namespace Impl

/// @brief Emplaces a `std::unique_ptr<T>` inside mContainer and returns a
/// reference to the allocated T instance.
/// @details Internally uses `ssvu::mkUPtr` and `std::unique_ptr<T>::get`.
/// @param mContainer Container of `std::unique_ptr<T>` where the newly created
...
/// @brief Emplaces a `std::unique_ptr<T>` inside a map-like mContainer and
/// returns a reference to the allocated T instance.
/// @details Internally uses `ssvu::mkUPtr` and `std::unique_ptr<T>::get`.
/// @param mContainer Map container of `std::unique_ptr<T>` where the newly
...
template <typename TArray, std::size_t TS>
inline constexpr std::size_t getCArraySize(TArray (&)[TS]) noexcept
{
    return TS;
}
...
inline auto mkUnorderedMap(TArgs&&... mArgs)
{
    ...
    using KeyType =
        MPL::L::Rename<std::common_type_t, MPL::FilterIdx<EvenFn, TArgs...>>;
    using ValueType =
        MPL::L::Rename<std::common_type_t, MPL::FilterIdx<OddFn, TArgs...>>;
