    ...
    namespace Impl
    {
        ...
        struct ToStrImpl
        {
            inline static auto toStr(const T& mX)
            {
                ...
                Impl::callStringifyImpl<false, false>(
                    Impl::getStringifyStream(), mX);
...
#undef SSVU_IMPL_FASTINTTOSTR_CONV
    ...
    inline void replace(std::string& mStr, const std::string& mFrom,
    ...
    /// @param mFrom String to replace.
    /// @param mTo Replacement string.
    inline void replaceAll(std::string& mStr, const std::string& mFrom,
                           const std::string& mTo)
    {
    ...
    inline bool beginsWith(const std::string& mStr,
                           const std::string& mStart) noexcept
    {
    ...
    inline bool endsWith(const std::string& mStr,
                         const std::string& mEnding) noexcept
    {
        return mStr.size() >= mEnding.size() &&
               mStr.compare(mStr.size() - mEnding.size(), mEnding.size(),
                            mEnding) == 0;
    }
    ...
    /// occurrence).
    inline auto getReplaced(std::string mStr, const std::string& mFrom,
                            const std::string& mTo)
    {
    ...
    /// occurrences).
    inline auto getReplacedAll(std::string mStr, const std::string& mFrom,
                               const std::string& mTo)
    {
