    ...
    namespace Console
    {
        namespace Impl
        {
            ...
            struct FmtCache
            {
                ...
                std::vector<std::string> styleCodeStrs, colorFGCodeStrs,
                    colorBGCodeStrs;
            ...
            inline auto& getFmtCache() noexcept
            {
                static FmtCache result;
                return result;
            }

            inline auto& getLastStyle() noexcept
            {
                return getFmtCache().lastStyle;
            }
            inline auto& getLastColorFG() noexcept
            {
                return getFmtCache().lastColorFG;
            }
            inline auto& getLastColorBG() noexcept
            {
                return getFmtCache().lastColorBG;
            }

            inline const auto& getStyleStr(Style mStyle) noexcept
            {
                return getFmtCache().styleCodeStrs[int(mStyle)];
            }
            inline const auto& getColorFGStr(Color mColor) noexcept
            {
                return getFmtCache().colorFGCodeStrs[int(mColor)];
            }
            inline const auto& getColorBGStr(Color mColor) noexcept
            {
                return getFmtCache().colorBGCodeStrs[int(mColor)];
            }

            inline const auto& getFmtStr() noexcept
            {
                static IgnoreManip result;
                result = {prefix + getStyleStr(getLastStyle()) + ";" +
                          getColorFGStr(getLastColorFG()) + ";" +
                          getColorBGStr(getLastColorBG()) + postfix};
                return result;
            }
            ...
            inline const auto& getStrStyle(Style mStyle) noexcept
            {
                getLastStyle() = mStyle;
                return getFmtStr();
            }
            inline const auto& getStrColorFG(Color mColor) noexcept
            {
                getLastColorFG() = mColor;
                return getFmtStr();
            }
            inline const auto& getStrColorBG(Color mColor) noexcept
            {
                getLastColorBG() = mColor;
                return getFmtStr();
            }
            inline const auto& getStrClear() noexcept
            {
                return getFmtCache().strClear;
            }
            ...
            inline const auto& getInfoImpl() noexcept
            {
                static InfoImpl result;
                return result;
            }
            ...
            namespace Info
            {
                ...
                {
                    return getInfoImpl().columns;
                }
                inline SizeT getRowCount() noexcept
                {
                    return getInfoImpl().rows;
                }
            }
        }
    }
