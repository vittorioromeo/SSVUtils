    ...
    namespace Impl
    {
        ...
        /*
                    inline void setColor(int mFGCode, int mBGCode)
                    {
                        ...
                        unsigned short wAttributes{((unsigned)mBGCode << 4) |
           (unsigned)mFGCode};
                        SetConsoleTextAttribute(getStorage().hConsole,
           wAttributes);
                    }
        ...
        struct StrStorage
        {
            ...
            inline void apply()
            {
                ...
                (
                (unsigned short)getColorCode((unsigned short)(lastColorFG) % 16)
                << 4) |
                (unsigned short)getColorCode(
                (unsigned short)(lastColorBG) % 16));
                SetConsoleTextAttribute(hConsole, wAttributes);
            }
        };

        inline StrStorage& getStorage() noexcept
        {
            static StrStorage storage;
            return storage;
        }
        /*
                    ...
                    inline const std::string& getStrStyle(Style) noexcept
           { getStorage().apply(); return getStorage().emptyString; }
                    inline const std::string& getStrColorFG(Color mColor)
           noexcept	{ getStorage().lastColorFG = mColor; getStorage().apply();
           return getStorage().emptyString; }
                    inline const std::string& getStrColorBG(Color mColor)
           noexcept	{ getStorage().lastColorBG = mColor; getStorage().apply();
           return getStorage().emptyString; }
                    */
