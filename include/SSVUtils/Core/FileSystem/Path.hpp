...
namespace ssvu
{
    namespace FileSystem
    {
        namespace Impl
        {
            ...
            {
                inline static bool get(const CStat&) noexcept { return true; }
            };
            template <>
        ...
        public:
            ...
            inline auto&
            operator=(T&& mPath) noexcept(noexcept(path = FWD(mPath)))
            {
                path = FWD(mPath);
                mustNormalize = true;
                return *this;
            ...
            template <typename T, SSVU_ENABLEIF_RA_IS_NOT(T, Path)>
            inline auto& operator+=(T&& mPath) noexcept(noexcept(path +=
                                                                 FWD(mPath)))
            {
                path += FWD(mPath);
                mustNormalize = true;
            ...
            inline std::string getFileNameNoExtensions() const
            {
                ...
                auto extBegin(
                    str.find_first_of('.', beginsWith(str, '.') ? 1 : 0));
        ...
        inline std::ostream& operator<<(std::ostream& mStream,
                                        const Path& mPath)
        {
...
#define ENABLEIF_ANY_PATH() \
    EnableIf<isSame<RmAll<T1>, Path>() || isSame<RmAll<T2>, Path>()>* = nullptr
