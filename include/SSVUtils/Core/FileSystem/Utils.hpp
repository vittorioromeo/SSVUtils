...
namespace FileSystem
{
    ...
    inline bool removeFile(const Path& mPath)
    {
        return std::remove(mPath.getCStr());
    }

    /// @brief Expands the `~` symbol in a path with the user's home path.
    /// @details Uses environment variables. Same logic as python's
    /// `os.path.expanduserpath`.
    ...
    inline Path getExpandedUserPath(Path mPath)
    {
        expandUserPath(mPath);
        return mPath;
    }
}
}
