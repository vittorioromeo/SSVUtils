...
namespace ssvu
{
    namespace Impl
    {
        ...
        template <bool TFmt, typename T>
        inline void
        printNonBold(std::ostream& mStream, const T& mValue,
                     Console::Color mColor = Console::Color::Default)
        {
        ...
        template <typename TItr, typename TFunc1, typename TFunc2>
        inline void repeatPenultimate(TItr mBegin, TItr mEnd, TFunc1 mFunc,
                                      TFunc2 mFuncSeparator)
        {
            repeatPenultimateImpl(
                mBegin, mEnd, mFunc, mFuncSeparator,
        ...
        inline void stringifyMapImpl(std::ostream& mStream, const T& mValue,
                                     const TSep& mSeparator)
        {
            auto itrBegin(std::begin(mValue));
...
#define SSVU_IMPL_DEFINE_BASE_STRINGIFIER(mType, mColor, mStyle, mPostfix)  \
    template <>                                                             \
    struct Stringifier<mType>                                               \
    {                                                                       \
        template <bool TFmt>                                                \
        inline static void impl(std::ostream& mStream, const mType& mValue) \
        {                                                                   \
            if(TFmt)                                                        \
                mStream << Console::setStyle(mStyle)                        \
                        << Console::setColorFG(mColor);                     \
            mStream << mValue;                                              \
            if(TFmt) mStream << mPostfix;                                   \
        }                                                                   \
    }

    SSVU_IMPL_DEFINE_BASE_STRINGIFIER(int, Console::Color::LightBlue,
                                      Console::Style::Bold, "");
    SSVU_IMPL_DEFINE_BASE_STRINGIFIER(long, Console::Color::LightBlue,
                                      Console::Style::Bold, "l");
    SSVU_IMPL_DEFINE_BASE_STRINGIFIER(unsigned int, Console::Color::LightBlue,
                                      Console::Style::Bold, "u");
    SSVU_IMPL_DEFINE_BASE_STRINGIFIER(unsigned long, Console::Color::LightBlue,
                                      Console::Style::Bold, "ul");
    SSVU_IMPL_DEFINE_BASE_STRINGIFIER(float, Console::Color::LightRed,
                                      Console::Style::Bold, "f");
    SSVU_IMPL_DEFINE_BASE_STRINGIFIER(double, Console::Color::LightRed,
                                      Console::Style::Bold, "d");
    SSVU_IMPL_DEFINE_BASE_STRINGIFIER(std::string, Console::Color::LightYellow,
                                      Console::Style::None, "");
...
#undef SSVU_IMPL_DEFINE_BASE_STRINGIFIER
    ...
    struct Stringifier<T*>
    {
        template <bool TFmt>
        inline static void
        impl(std::ostream& mStream, const T* mValue,
             EnableIf<!isSame<RmConst<T>, char>()>* = nullptr)
        {
            Impl::printBold<TFmt>(mStream, "[", Console::Color::Blue);
            Impl::printFmt<TFmt>(
                mStream, mValue != nullptr ? static_cast<const void*>(mValue)
                                           : "nullptr",
