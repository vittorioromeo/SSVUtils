...
public:
    ...
    std::size_t getTicks() const noexcept;

    template <typename T = FT>
    inline T getTotalSecs() const noexcept
    {
        return toNum<T>(getFTToSeconds(total));
    }
    template <typename T = FT>
    inline T getCurrentSecs() const noexcept
    {
        return toNum<T>(getFTToSeconds(current));
    }
};
} // namespace ssvu
