...
/// @brief Returns a reference to a static  thread_local `bool` used
/// to keep track if there was more than one benchmark on the stack.
auto& hadNested() noexcept;

/// @brief Returns a reference to the thread_local benchmark group
/// map.
auto& getGroupMap() noexcept;

/// @brief Returns a reference to the top of the stack. Assumes that
/// the stack is not empty.
auto& getLastDataRef();

/// @brief Creates and pushes a new benchmark on the stack. If it's
/// nested, sets `hasNested()` to true.
void start(std::string mTitle);

/// @brief Pops and returns the top of the stack.
Data getEndData();

/// @brief Pops the top of the stack. Logs the benchmark on
/// `ssvu::lo()`.
/// @details Automatically adds a newline between "groups" of nested
/// benchmarks.
void endLo();
