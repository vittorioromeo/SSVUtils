...
#ifndef SSVU_IMPL_CONTAINER_VECSORTED
#define SSVU_IMPL_CONTAINER_VECSORTED
...
public:
    ...
    inline VecSorted(std::initializer_list<T>&& mIL) : data{std::move(mIL)}
    {
        sort(data, cmp);
    }
    ...
    inline auto& operator[](std::size_t mI) noexcept
    {
        return data[mI];
    }

    /// @brief Returns a const reference to the item stored at index `mI`.
    inline const auto& operator[](std::size_t mI) const noexcept
    {
        return data[mI];
    }
};
