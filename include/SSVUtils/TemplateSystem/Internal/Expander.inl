    ...
    namespace TemplateSystem
    {
        ...
        inline bool Expander::replaceSection()
        {
            ...
            for(const auto* cd(&dict); cd != nullptr; cd = cd->parentDict)
            {
                ...
                for(auto i(0u); i < dictVec.size() - 1; ++i)
                    dictVec[i].expandImpl(src, bufResult, bufKey,
                        sectIdxCntStart, sectIdxCntEnd, true, settings);

                // Non-separated expansion
                dictVec[dictVec.size() - 1].expandImpl(src, bufResult, bufKey,
                    sectIdxCntStart, sectIdxCntEnd, false, settings);
        ...
        inline bool Expander::expand()
        {
            ...
            for(; idx < idxEnd; ++idx)
            {
                ...
                if(type == Type::Normal)
                {
                    if(replace()) result = true;
                    continue;
                }
                if(type == Type::Separator)
                {
                    if(separate) bufResult += bufKey;
                    continue;
                }
