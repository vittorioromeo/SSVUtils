...
namespace ssvu
{
	namespace TemplateSystem
	{
		class Dictionary
		{
			...
			private:
				...
				template<typename TKey> class Proxy
				{
					...
					public:
						...
						template<typename T> inline auto& operator=(T&& mReplacement) { dict.replacements[key] = FWD(mReplacement); return *this; }
						template<typename T> inline auto& operator+=(T&& mDict)	{ dict.sections[key].emplace_back(FWD(mDict)); return *this; }
				};
				...
				template<typename T> inline void initImpl(T&& mKey, std::string&& mReplacement)			{ replacements[FWD(mKey)] = mv(mReplacement); }

				// Init section replacement
				template<typename T> inline void initImpl(T&& mKey, const DictVec& mDicts)	{ sections[FWD(mKey)] = mDicts; }
				template<typename T> inline void initImpl(T&& mKey, DictVec&& mDicts)		{ sections[FWD(mKey)] = mv(mDicts); }
			...
			public:
				template<typename... TArgs> inline Dictionary(TArgs&&... mArgs) { init(FWD(mArgs)...); }
				...
				template<typename T> inline auto operator[](T&& mKey) noexcept { return Proxy<T>{FWD(mKey), *this}; }
		};
	}
}
