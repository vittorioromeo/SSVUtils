...
private:
    ...
    {
        replacements[FWD(mKey)] = std::move(mReplacement);
    }
    ...
    {
        sections[FWD(mKey)] = std::move(mDicts);
    }
...
public:
    ...
    inline std::string getExpanded(
        std::string mSrc, Settings mSettings = Settings::EraseUnexisting)
    {
        refreshParents();
