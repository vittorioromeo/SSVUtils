...
namespace ssvu
{
    namespace Encryption
    {
        namespace Impl
        {
            ...
            SSVU_INLINE std::string
            Base64Encode(unsigned char const* bytes_to_encode,
                         unsigned int in_len)
            {
                std::string ret;
                int i{0}, j{0};
            ...
            SSVU_INLINE std::string Base64Encode(const std::string& mStr)
            {
                return Base64Encode(
                    reinterpret_cast<const unsigned char*>(mStr.c_str()),
                    mStr.size());
            }
        }
    }
